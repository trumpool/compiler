...
VarDecl : IntTypeDecl VarDefs ";"              {
    auto typeDef = $$->children[0];
    auto firstIdent = $$->children[1];
    AST* def = F.makeNode("VarDef",  firstIdent);
    $$ = F.makeNode("VarDecl", typeDef, def);
    $$->absorb($2);
    }
    // | IntTypeDecl ArrayLengths VarDefs ";"     {$$ = F.makeNode("VarDecl"); $$->add_children(F.makeNode("VarDef", $1->children[0],  $1->children[1], $2)); $$->absorb($3);}
    | IntTypeDecl ArrayLengths VarDefs ";"     {
        $$ = F.makeNode("VarDecl");
        $$->add_children($1->children[0]);
        AST *varDef = F.makeNode("VarDef", $1->children[1], $2);
        $$->add_children(varDef);
        $$->absorb($3);
    }
    | IntTypeDecl "=" InitVal VarDefs ";"      {$$ = F.makeNode("VarDecl"); $$->add_children($1->children[0],F.makeNode("VarDef",  $1->children[1], $2, $3)); $$->absorb($4);}
    ;
...
FuncParams : FuncParam          {$$ = F.makeNode("FuncParams", $1);}
    ...
    {
        $$ = F.makeNode("FuncParams");
        $$->absorb($1);
        $$->add_children($3);
        }
    ;

FuncParam : IntTypeDecl         {$$ = F.makeNode("FuncParam"); $$->absorb($1);}         
    | IntTypeDecl "[" "]"       {$$ = F.makeNode("FuncParam_Array");$$->absorb($1);}
    | IntTypeDecl "[" "]" ArrayLengths  {$$ = F.makeNode("FuncParam_Array");$$->absorb($1); $$->add_children($4);}
    ;
...
BlockItems : BlockItem                  {$$ = F.makeNode("BlockItems", $1);}
    | BlockItems BlockItem              {$$ = $1->add_children($2);}
    ;
